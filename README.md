...
- **`Xliff::Header` models an element name and its attributes only** – a header's text content and any nested child elements are not preserved on round-trip. Namespace prefixes are also mishandled: a non-`xml:` prefix on the element name is dropped, and a prefixed attribute is re-emitted without its namespace declaration (producing invalid XML); see [#18](https://github.com/Automattic/xliff/issues/18).
