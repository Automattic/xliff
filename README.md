...
- **`Xliff::Header` models an element name and its attributes only** – a header's text content and any nested child elements are not preserved on round-trip. It also doesn't bind XML namespaces: a non-`xml:` prefix is dropped from a parsed element name, and any non-`xml:` prefix (on an element or attribute name, whether parsed or built by hand) is serialized without its `xmlns` declaration – producing invalid XML. The `xml:` prefix is the exception (it's always bound). See [#18](https://github.com/Automattic/xliff/issues/18).
