...
- **`Xliff::Header` models an element name and its attributes only** – a header's text content and any nested child elements are not preserved on round-trip. It also doesn't bind XML namespaces. On parse, a non-`xml:` prefix on an element name is dropped (the element re-homes into the default namespace), while a non-`xml:` prefix on an attribute name is kept but re-emitted without its `xmlns` declaration – producing invalid XML. Building a header by hand instead rejects any non-`xml:` prefix outright, since the library has no way to declare it. The `xml:` prefix is the exception (it's always bound). See [#18](https://github.com/Automattic/xliff/issues/18).
